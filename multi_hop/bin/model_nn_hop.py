...
class Model():
    ...
    def build_model(self):
        ...
        self.input_net['label'] = tf.placeholder(tf.float32,[None,2])
        ...
        #hop1 += last_query
        #self.att2,hop2 = hopping(hop1,self.input_net['audio_length'],audio_output)
        #hop2 += hop1 
        #self.att3,hop3 = hopping(hop2,self.input_net['audio_length'],audio_output)
        #hop3 += hop2 
        #self.att4,hop4= hopping(hop3,self.input_net['audio_length'],audio_output)
        #hop4 += hop3 
        #self.att5,hop5= hopping(hop4,self.input_net['audio_length'],audio_output)
        ...
        self.output_net['predict'] = tf.nn.softmax(self.output_net['predict'])
        self.output_net['accuracy'] = tf.equal(tf.argmax(self.input_net['label'],1),tf.argmax(self.output_net['predict'],1))
        self.output_net['accuracy'] = tf.reduce_mean(tf.cast(self.output_net['accuracy'], tf.float32))
        self.var = tf.trainable_variables()
        self.lossL2 = tf.add_n([ tf.nn.l2_loss(v) for v in self.var if 'bias' not in v.name ]) *0.001
        self.output_net['loss'] = self.output_net['label_loss'] + self.lossL2
    ...
    def train(self,audio,query,audio_length,query_length,label):
        _, loss = self.sess.run([self.joint_opti, \
                    (self.output_net['label_loss'],self.output_net['accuracy'])], \
    ...
    def test(self,audio,query,audio_length,query_length,label):
        return self.sess.run([self.output_net['predict'],self.output_net['accuracy'],self.output_net['label_loss']],feed_dict={self.input_net['audio']:audio,self.input_net['query']:query,self.input_net['audio_length']:audio_length,self.input_net['query_length']:query_length,self.input_net['label']:label})
