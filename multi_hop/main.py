import sys
model_name = ''
import bin.model_nn_hop as model
...
if __name__ =='__main__':
    ...
    def train(alpha,beta,train_num):
        ...
        while data_num < train_num:
            ...
            acc = (acc*data_num+batch_loss[1]*data_gen_num)/(data_num+data_gen_num)
            data_num+=data_gen_num
            sys.stdout.write('total_class_loss:'+str(loss_class)+' acc: '+str(acc)+' '+str(data_num)+'/'+str(train_num)+'\r')
            sys.stdout.flush()
        td=time.time()
        print '\n',
        print 'cost_time=',td-ts
        train_record.append((loss_class,acc))
    ...
    def validation(label,batch_size,query_map,doc_map,data_name,dir_label):
        ...
        while data_num < test_num:
            ...
            val_acc = (val_output[1]*data_gen_num + val_acc *data_num)/ (data_gen_num+data_num)
            val_loss = (val_output[2]*data_gen_num + val_loss*data_num)  /(data_gen_num+data_num) 
            data_num += data_gen_num
            sys.stdout.write('class_loss:'+str(val_loss)+' acc:'+str(val_acc)+' '+str(data_num)+'/'+str(test_num)+'\r')
            sys.stdout.flush()
