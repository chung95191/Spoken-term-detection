...
def a2v_feat_gen(data_list,batch_size,max_len,feat_map):
    count=0
    feat_batch=[]
    len_batch=[]
    for i in data_list:
        count+=1
        feat = feat_map[i][0]
        feat_mask = feat_map[i][1]
        feat_batch.append(feat)
        len_batch.append(feat_mask)
        if count == batch_size:
            feat_batch = np.array(feat_batch).reshape(batch_size,max_len,39)
            len_batch = np.array(len_batch)
            yield feat_batch,len_batch
            count=0
            feat_batch=[]
            len_batch=[]
    residual_num=len(len_batch)
    if residual_num>0:
        feat_batch = np.array(feat_batch).reshape(residual_num,max_len,39)
        len_batch = np.array(len_batch)
        yield feat_batch,len_batch
